...
plotStates = find(pct > 0);
if ~isempty(plotStates)
    h = pie(axPie, pct(plotStates));
    title(axPie, 'Activity budget', 'FontWeight', 'bold');

    patchHandles = findobj(h, 'Type', 'Patch');
    patchHandles = flipud(patchHandles);
    for k = 1:min(numel(patchHandles), numel(plotStates))
        patchHandles(k).FaceColor = stateColors(plotStates(k),:);
    end

    legtxt = strings(1,numel(plotStates));
    for k = 1:numel(plotStates)
        ss = plotStates(k);
        legtxt(k) = sprintf('%s (%.1f%%)', stateFigNames{ss}, pct(ss));
    end
